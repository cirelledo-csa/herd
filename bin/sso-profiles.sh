...
echo
echo "enter an optional prefix for profiles, eg my-org- (include a terminating dash as profiles will be named $prefix$account_name-$role)"
read prefix
...
do
	...
	for role in $(aws sso list-account-roles  --account-id $account_id --access-token $token  | jq -r '.roleList[].roleName'); do
		echo "[profile $prefix$account_name-$role]";
		echo output = json;
